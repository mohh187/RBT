...
**جولة المجسم والطاولة والنصوص (نفس اليوم، `252e899`، نُشرت hosting):** (1) **فخ جديد مثبت — أضفه لقائمتك الذهنية: `className` على عنصر مخصص (model-viewer) يُكتب حرفياً `classname` في React 18** — كل CSS ميت والعارض يسقط لكانفس 300x150 يتجاهل كل الأقراص؛ الصحيح `class=` (موقعان أُصلحا: زينة EdtDecorPiece وعارض AR) + حزام أمان `.edt-dec-p model-viewer` في menuwall.css + spin يتحول لـauto-rotate أصلي بمعدل `decorSpinRate` (المعاينة والمحرك معاً) + بطاقة الزينة تشغّل المجسم حياً (`useMiniModelViewer` + `.mdx-mv`). (2) **TablePaint.jsx** مكوّن رسم واحد للطاولة (المحرك + بطاقة الإعدادات) — المعاينة كانت تطبّق التحويل الحر على الصندوق كله فتكذب؛ dim انتقل لغلاف الأثاث (تعديل جزئي مقصود على قرار 7de19d8)؛ shade صار حياً على الطاولات الفاتحة/الصور (b0 = 21.6+shade*24، الافتراضي 0.35 يعطي 30 بالضبط)؛ radius انتقل لصندوق القص عبر `tableBoxStyle` (كان أخرس تحت 18px). (3) `minBody` (dvh، ضمن TABLE_STAGE_KEYS، `--edt-stg-bodymin` بfallback `auto` حصراً) يوحّد شكل الطاولة بين الأصناف. (4) **menuStage**: `STAGE_BLOCKS` (11 كتلة + ar مكاناً فقط) + `STAGE_TEXT_RANGE` (dx منطقية % / dy px، margins لا transforms) + `resolveStageBlocks/stageBlockProps` + إعادة هيكلة جسم المسرح لخريطة nodes بمفاتيح (الافتراضي = تسلسل المصدر حرفياً؛ مجموعات الإضافات في Fragment بمفتاح) + بطاقة `#set-menustage` (سحب ترتيب + محاذاة + إزاحتان لكل كتلة). `mergeOrder` مشترك بين ترتيبي الرئيسية والنافذة.
**جولة الطاولة المستقلة وزر الجدار (نفس اليوم، `68a412f`، نُشرت hosting + imageTo3d):** (1) **سياسة «غرفة واحدة طاولة واحدة» نُقضت بطلب المالك الصريح**: `item.table` جزئي (كل مفاتيح menuTable + stage{}) يُدمج فوق طاولة الغرفة عبر `resolveTable(tenant,{variant,item})`؛ القائمة (EdtSection يستقبل tenant ويحلّ لكل صنف يحمل table) والمسرح كلاهما؛ محرر الصنف قسم «طاولة هذا الصنف» `#ie-table` (الغرفة/مستقلة/بدون + `tablePayload/cleanItemTable` في save + draftItem). (2) زر AR: `menuStage.blocks.ar.pos='wall'` (`STAGE_AR_POS_IDS`، `data-blk-pos`) يرفعه مطلقاً فوق الطوب متوسطاً (`.edt-stg-media` صارت relative على الجوال فقط — سطح المكتب sticky/static كما هي والزر يرسو على الجذر)؛ مثبت للمنشأة الحية. (3) `featuredScale` (tenant، `FEATURED_RANGE.scale` 0.6-1.8، `--feat-scale` يضرب عرض البطاقة والصورة) + منزلق في بطاقة المميزة + القوائم الأربع. (4) **`tenant.ar3dUnlimited === true` يتجاوز حدَّي التوليد** (الشهري + 2/صنف) مع بقاء حارس رصيد Meshy — **مفعّل حالياً لصاج السمك بطلب مؤقت؛ الإرجاع: احذف الحقل أو اجعله false، بلا نشر**.
