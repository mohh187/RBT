...
// The venue's WALL — the brick backdrop the editorial menu sets its dishes
...
// ============================ THE MENU WALL ================================
// The brick backdrop behind the dishes. It shipped fully controllable in the
// renderer but with NO screen to control it, so the only way to change it was
// hand-editing Firestore («لا اجد اعدادات الطوب والخلفية في الاعدادات»). The
// card built from the constants below closes that gap and exposes every key the
// contract defines — nothing about the wall is reachable only through the
// database any more.
//
// Stored as ONE object at `tenant.menuWall`. These defaults are resolveWall()'s
// own fallbacks written out, so «إرجاع الافتراضي» reproduces exactly the wall a
// venue that never opened this card would have seen.
const WALL_DEFAULTS = {
  pattern: 'running', url: '', finish: 'clean',
  color: '#8a4a2c', mortarColor: '#b9a893',
  scale: WALL_RANGE.scale.dflt, opacity: WALL_RANGE.opacity.dflt,
...
const WALL_TILE_VIEW = { scale: 0.34, opacity: 1, blur: 0, blend: 'normal', filter: '', tint: '', tintAmount: 0 }
// [key, ar, en, unit] — every one bounded by WALL_RANGE, never by a local number.
const WALL_SLIDERS = [
  ['grout', 'عرض اللحام (المونة بين الوحدات)', 'Joint width', 'px'],
  ['mortar', 'وضوح اللحام', 'Joint contrast', 'pct'],
...
// force the venue to throw away most of a photograph of its own room.
const CROP_KINDS = {
...
/**
 * One wall face, painted by the THEME'S OWN generator.
 *
 * `paint` is EditorialLayout's wallPaint(), fetched at runtime (see the loader
 * in the component). Reusing it is the whole point: a second generator written
 * here would drift from the menu the first time either side was touched, and
 * the venue would be choosing from tiles the menu never draws.
 *
 * Until the generator answers — and if it is ever unreachable — this falls back
 * to the contract's own mood-only style over the venue's clay, so colour,
 * opacity, blur and blend still read while the bond does not.
 */
function wallFaceStyle(paint, cfg) {
  const w = resolveWall({ menuWall: cfg })
  if (!w) return null
  if (paint) return paint(w)
  const s = wallStyle(w) || {}
  return { ...s, backgroundColor: w.color }
}
...
export default function Settings() {
  ...
  // ===== THE MENU WALL (tenant.menuWall) =====
  // Draft-free: every control writes straight to the tenant, because the wall is
  // the one thing the venue was previously editing by hand in the database and a
  // half-saved brick is worse than none. Sliders and colour inputs go through the
  // shared 400ms debounce; discrete picks save immediately.
  const [wallCfg, setWallCfg] = useState(() => ({ ...WALL_DEFAULTS, ...(tenant?.menuWall || {}) }))
  // Keyed on the SAVED object, not on the tenant: an unrelated field's save must
  // not bounce this card back mid-edit, but undo/redo and the AI assistant must.
  const wallSavedKey = JSON.stringify(tenant?.menuWall || null)
  useEffect(() => { setWallCfg({ ...WALL_DEFAULTS, ...(tenant?.menuWall || {}) }) }, [wallSavedKey]) // eslint-disable-line react-hooks/exhaustive-deps
  // The theme's own tile generator, fetched only once this card is on screen so
  // the admin bundle never carries the whole editorial menu. See wallFaceStyle().
  //
  // `wallPainterGone` is not defensive noise: if the theme ever stops exporting
  // wallPaint, every bond tile collapses to the same flat clay swatch and the
  // picker LOOKS broken while working perfectly. The venue is told instead of
  // being left to guess — and the message can never flash on a normal load,
  ...
  useEffect(() => {
    ...
    import('../../components/menuThemes/EditorialLayout.jsx')
      ...
      .catch(() => { if (alive) setWallPainterGone(true) })
    return () => { alive = false }
  }, [wallCardLive, wallPainter, wallPainterGone])
  const writeWall = async (patch, debounced) => {
    const next = { ...wallCfg, ...patch }
    setWallCfg(next)
    if (debounced) { commitPosBg({ menuWall: next }); return }
    try { await saveNow({ menuWall: next }); updateTenantLocal({ menuWall: next }); toast.success(t('saved')) } catch (_) { toast.error(t('error')) }
  }
  const resetWall = async () => {
    const next = { ...WALL_DEFAULTS }
    setWallCfg(next)
    try { await saveNow({ menuWall: next }); updateTenantLocal({ menuWall: next }); toast.success(t('saved')) } catch (_) { toast.error(t('error')) }
  }
  // A picker tile is the venue's CURRENT wall with one key swapped and the mood
  // stripped, so the tiles differ only by the thing they are offering.
  //
  // MEMOISED ON WHAT THEY ACTUALLY SHOW, and deliberately not on the whole
  // config: sixteen tiles is sixteen generated SVG walls, and regenerating all
  // of them on every tick of the opacity slider — which the tiles do not even
  // display — would make the card stutter under the venue's finger. Only the
  // large preview follows every key.
  const wallTileKey = [wallPainter ? 'p' : '-', wallCfg.pattern, wallCfg.finish, wallCfg.color, wallCfg.mortarColor, wallCfg.grout, wallCfg.mortar, wallCfg.url].join('|')
  const wallTiles = useMemo(() => {
    const view = { ...wallCfg, ...WALL_TILE_VIEW }
    const patterns = {}
    WALL_PATTERNS.forEach((p) => {
      patterns[p.id] = p.id === 'none'
        ? null
        : p.id === 'image'
          ? (wallCfg.url ? { backgroundImage: `url(${wallCfg.url})`, backgroundSize: 'cover', backgroundPosition: 'center' } : null)
          : wallFaceStyle(wallPainter, { ...view, pattern: p.id })
    })
    const finishes = {}
    // on «صورتي» the finish still acts — it is drawn OVER the photograph — so the
    // tile shows the real thing at full size rather than a tiled thumbnail
    WALL_FINISHES.forEach((f) => {
      finishes[f.id] = wallFaceStyle(wallPainter, wallCfg.pattern === 'image' ? { ...view, finish: f.id, scale: 1 } : { ...view, finish: f.id })
    })
    return { patterns, finishes }
  ...
  return (
    <div className="page stack settings-page" style={{ gap: 'var(--sp-3)' }}>
      ...
      {cropState && (() => {
        // one table per upload kind (CROP_KINDS) instead of a ternary chain, so a
            ...
            {planAllows(tenant, 'themes') ? (
              <>
                ...
                {/* 6b. THE MENU WALL — the brick backdrop the dishes hang on.
                    ...
                    blur, blend, filter and tint. Each picker tile is painted by
                    the THEME'S OWN generator, so what is chosen is what the menu
                    draws. Option rows WRAP — a non-scrolling chip strip on a
                ...
                <div id="set-menuwall" className="card card-pad stack" style={{ gap: 12 }}>
                  ...
                  <input ref={wallFileRef} type="file" accept="image/*" hidden onChange={onWallFile} />
                  {wallPainterGone && (
                    <p className="xs" style={{ margin: 0, color: 'var(--warning)' }}>
                      {ar
                        ? 'معاينات الأشكال غير متاحة الآن، فتظهر المربّعات بلون الطين وحده. الاختيار والحفظ يعملان كالمعتاد، والمنيو يرسم الشكل الذي تختاره.'
                        : 'The pattern previews are unavailable right now, so the tiles show the clay colour alone. Picking and saving work as usual, and the menu still draws the bond you choose.'}
                    </p>
                  )}
                  ...
                  <div className="stack" style={{ gap: 6 }}>
                    ...
                    <div className="mwall-grid">
                      {WALL_PATTERNS.map((p) => {
                        ...
                        const face = wallTiles.patterns[p.id]
                        return (
                          <button
                            key={p.id}
                            type="button"
                            className="mwall-card"
                            data-on={on ? 'true' : 'false'}
                            aria-pressed={on}
                            onClick={() => {
                              // picking «صورتي» with nothing uploaded would blank the
                              // wall on a live menu, so it opens the picker instead
                              if (p.id === 'image' && !wallCfg.url) { wallFileRef.current?.click(); return }
                              writeWall({ pattern: p.id })
                            }}
                          >
                            <span className="mwall-face" style={face || undefined} data-empty={face ? undefined : 'true'}>
                              {p.id === 'image' && uploading === 'wall'
                                ? <div className="spinner" />
                                : face ? null : <Icon name={p.id === 'image' ? 'image' : 'close'} size={15} />}
                            </span>
                            <span className="mwall-name">{ar ? p.ar : p.en}</span>
                          </button>
                        )
                      })}
                    </div>
                    ...
                    <>
                      <div className="stack" style={{ gap: 6 }}>
                        ...
                        <div className="mwall-grid">
                          {WALL_FINISHES.map((f) => {
                            ...
                            const face = wallTiles.finishes[f.id]
                            return (
                              <button key={f.id} type="button" className="mwall-card" data-on={on ? 'true' : 'false'} aria-pressed={on} onClick={() => writeWall({ finish: f.id })}>
                                <span className="mwall-face" style={face || undefined} />
                                <span className="mwall-name">{ar ? f.ar : f.en}</span>
                              </button>
                            )
                          })}
                        </div>
                      ...
                      <div className="row wrap" style={{ gap: 16, alignItems: 'center', borderTop: '1px solid var(--border)', paddingTop: 10 }}>
                        <label className="row" style={{ gap: 6, cursor: 'pointer', alignItems: 'center' }}>
                          <span className="xs faint">{ar ? 'لون الطوب' : 'Brick colour'}</span>
                          <input type="color" value={wallCfg.color} onChange={(e) => writeWall({ color: e.target.value }, true)} style={{ width: 42, height: 32, border: '1px solid var(--border)', borderRadius: 8, background: 'none', cursor: 'pointer' }} aria-label={ar ? 'لون الطوب' : 'Brick colour'} />
                          <span className="xs bold num">{String(wallCfg.color).toUpperCase()}</span>
                        </label>
                        <label className="row" style={{ gap: 6, cursor: 'pointer', alignItems: 'center' }}>
                          <span className="xs faint">{ar ? 'لون المونة (اللحام)' : 'Mortar colour'}</span>
                          <input type="color" value={wallCfg.mortarColor} onChange={(e) => writeWall({ mortarColor: e.target.value }, true)} style={{ width: 42, height: 32, border: '1px solid var(--border)', borderRadius: 8, background: 'none', cursor: 'pointer' }} aria-label={ar ? 'لون المونة' : 'Mortar colour'} />
                          <span className="xs bold num">{String(wallCfg.mortarColor).toUpperCase()}</span>
                        </label>
                      </div>
                        ...
                        {WALL_SLIDERS.map(([key, la, le, unit]) => {
                          ...
                          return (
                            <div key={key} className="field" style={{ gap: 2 }}>
                              ...
                              <input type="range" min={R.min} max={R.max} step={R.step} value={v} onChange={(e) => writeWall({ [key]: Number(e.target.value) }, true)} style={{ width: '100%' }} />
                            </div>
                          )
                        })}
                      </div>
                      ...
                      <div className="row wrap" style={{ gap: 8, borderTop: '1px solid var(--border)', paddingTop: 10 }}>
                        <div className="field grow" style={{ minWidth: 170 }}>
                          <label>{ar ? 'مزج الجدار مع لوحة الثيم' : 'Blend with the theme canvas'}</label>
                          <select className="select" value={wallCfg.blend} onChange={(e) => writeWall({ blend: e.target.value })}>
                            {BLEND_MODES.map((b) => <option key={b.id} value={b.id}>{ar ? b.ar : b.en}</option>)}
                          </select>
                        </div>
                        <div className="field grow" style={{ minWidth: 170 }}>
                          <label>{ar ? 'فلتر الجدار' : 'Wall filter'}</label>
                          <select className="select" value={wallCfg.filter} onChange={(e) => writeWall({ filter: e.target.value })}>
                      ...
                      <div className="row wrap" style={{ gap: 16, alignItems: 'center' }}>
                        <label className="row" style={{ gap: 6, cursor: 'pointer', alignItems: 'center' }}>
                          <span className="xs faint">{ar ? 'صبغة فوق الجدار' : 'Tint over the wall'}</span>
                          <input type="color" value={wallCfg.tint || '#000000'} onChange={(e) => writeWall({ tint: e.target.value, tintAmount: wallCfg.tintAmount > 0 ? wallCfg.tintAmount : 0.2 }, true)} style={{ width: 42, height: 32, border: '1px solid var(--border)', borderRadius: 8, background: 'none', cursor: 'pointer' }} aria-label={ar ? 'لون الصبغة' : 'Tint colour'} />
                          <span className="xs bold num">{String(wallCfg.tint || '#000000').toUpperCase()}</span>
                        </label>
                        <div className="field grow" style={{ minWidth: 170 }}>
                          <label>{ar ? 'قوّة الصبغة' : 'Tint strength'} · <span className="num">{Math.round((Number(wallCfg.tintAmount) || 0) * 100)}%</span></label>
                          <input type="range" min="0" max="1" step="0.05" value={Number(wallCfg.tintAmount) || 0} onChange={(e) => writeWall({ tintAmount: Number(e.target.value) }, true)} style={{ width: '100%' }} />
                        </div>
                        {(wallCfg.tint || Number(wallCfg.tintAmount) > 0) && (
                          <button type="button" className="btn-link xs" style={{ color: 'var(--danger)' }} onClick={() => writeWall({ tint: '', tintAmount: 0 })}>{ar ? 'إزالة الصبغة' : 'Remove the tint'}</button>
                        )}
                      </div>
                      ...
                      <div className="stack" style={{ gap: 6, borderTop: '1px solid var(--border)', paddingTop: 10 }}>
                        ...
                        <div className="mwall-stage">
                          ...
                          <div className="mwall-dish">
                            {wallSample?.imageUrl
                              ? <img src={wallSample.imageUrl} alt="" className="mwall-dish-img" />
                              : <span className="mwall-dish-plate" aria-hidden="true" />}
                            <b className="mwall-dish-name">{wallSample ? pickLang(wallSample, 'name', lang) : (ar ? 'اسم الطبق' : 'Dish name')}</b>
                            <span className="mwall-dish-price num">{Number(wallSample?.price || 0).toFixed(2)} {currency}</span>
                          </div>
                        </div>
