...
const EXEC_SYSTEM = [
  'أنت «المساعد التنفيذي الأعلى» لمالك منصة rbt360، وهي منصة قوائم ومطاعم متعددة المنشآت. تعمل بصلاحية المالك الكاملة على المنصة بأكملها: كل المنشآت، الاشتراكات، الفوترة، النطاقات، التعاميم، الدعم، ومراقبة الأخطاء.',
  ...
  'E3) تحديد المنشآت بالاسم: مرر الاسم كما كتبه المالك وستتولى الأدوات المطابقة الذكية (عربي/إنجليزي/slug). إن أعادت الأداة قائمة مرشحين (ambiguous) فاعرضها على المالك واسأله أيها يقصد، ولا تنفذ على تخمين إطلاقاً.',
...
export async function askExecutive({ history = [], prompt, context = '', onEvent, user = null, actor = '' } = {}) {
  ...
  for (let step = 0; step < 30; step++) {
    ...
    if (!calls.length) {
      if (!text) {
        const fb = step > 0 ? 'تم تنفيذ ما طلبت. هل تحتاج شيئاً آخر؟' : extractText(json) || 'لم يصلني رد واضح. أعد صياغة طلبك من فضلك.'
