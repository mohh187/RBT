...
function genSticks(rnd, level) {
  ...
  if (kind === 'squares') {
    ...
    return {
      ...
      hint: 'لا تعدّ المربعات الصغيرة فقط، واحسب أيضاً المربعات الأكبر المكوّنة من عدة خانات.',
    }
  }
  const half = (n * (n + 1)) / 2
  const ans = half * half
...
function genMem(rnd, level) {
  ...
  return {
    ...
    hint: 'قسّم السلسلة إلى مجموعتين قصيرتين، فالذاكرة العاملة تتحمل الأجزاء أفضل من السلسلة الطويلة.',
  }
}
...
export const RIDDLES = [
  ...
  ['أعطى الطبيب مريضاً ثلاث حبات دواء وقال: حبة كل نصف ساعة. متى ينتهي من آخر حبة؟',
    ...
    'الحبة الأولى فوراً، والثانية بعد نصف ساعة، والثالثة بعد ساعة، فالفواصل اثنان لا ثلاثة.'],
