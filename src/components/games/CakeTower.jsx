// «برج الكيك» — CakeTower: a staged cake stacker. A cake layer slides over the
// tower; tap to drop it. The overhang is sliced off and tumbles away; a pixel
// perfect drop pays a combo bonus, flashes gold and hands back a sliver of
// width. Every few layers CLEAR A STAGE: the tower gets a fresh flavour, the
// slide speeds up and — from stage three — a breeze pushes the layer off its
// line, so timing alone stops being enough. A total miss ends the round.
//
// STAGES (مراحل): a real difficulty ladder. Numbered stages, a visible stage
// pill, a between-stages beat (the layer holds while the new-stage banner
// shows), and progress saved through onProgress so a guest who steps away
// resumes at the same stage with their score. A deeper run simply scores more.
//
...
const LAYERS_PER_STAGE = 5   // stacks needed to clear a stage
const STAGE_HOLD = 1.05      // seconds the layer waits while the stage banner shows

// sponge / cream pairs — deliberately warm and edible-looking; one per stage
const PALETTE = [
  ...
  { sponge: '#6fb0c9', cream: '#e9f7fb', crumb: '#4d8aa2' },
]
const FLAVORS = ['فانيلا', 'شوكولاتة', 'تمر', 'فستق', 'توت', 'قهوة', 'كراميل', 'جوز الهند', 'ليمون', 'عسل']
const STAGE_NAMES = [
  'كيك الإسفنج', 'الشوكولاتة', 'التمر والقشطة', 'الفستق الحلبي',
  'التوت البري', 'قهوة الموكا', 'الكراميل المملّح', 'جوز الهند',
]
const stageName = (s) => STAGE_NAMES[(s - 1) % STAGE_NAMES.length] + (s > STAGE_NAMES.length ? ` ${fmt(s)}` : '')

// per-stage difficulty curve
const stageSpeed = (s) => 0.46 + (s - 1) * 0.05          // × board width / second base
const stageWind = (s) => (s < 3 ? 0 : (s - 2) * 0.10)     // sinusoidal drift amplitude
...
export default function CakeTower({
  ...
  // a valid saved run for this game (continue where the guest stopped)
  const saved = useMemo(() => {
    const s = resumeState
    return s && s.game === GAME_ID && s.v === PROG_V && Number(s.stage) > 0 ? s : null
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [])
  const startCfgRef = useRef(null)
  ...
  const [banner, setBanner] = useState(null) // { stage, name }
  ...
  useEffect(() => {
    ...
    const st = {
      ...
      cam: 0, camTarget: 0, flash: 0, shake: 0, celebrate: 0,
      score: 0, combo: 0, stage: 1, stageLayers: 0, hold: 0, windT: 0,
      bg: null, glow: null, toast: { t: 0, txt: '', x: 0, y: 0 },
    }
    stRef.current = st
    ...
    const resize = () => {
      ...
      st.w = w
      st.h = h
      // type + layer height scale with the screen: readable on a phone, grand on a TV
      st.lh = Math.max(16, Math.min(46, h * 0.058))
    ...
    const baseW = () => Math.min(st.w * 0.62, Math.max(150, st.w * 0.42))

    const mkMov = (n) => {
      const prev = st.layers[st.layers.length - 1]
      const w = prev ? prev.w : baseW()
      const fromLeft = n % 2 === 1
      const speed = st.w * stageSpeed(st.stage)
      return {
        x: fromLeft ? 0 : Math.max(0, st.w - w),
        w, dir: fromLeft ? 1 : -1, speed,
        ci: (st.stage - 1) % PALETTE.length, label: pickLabel(n),
      }
    }

    const syncStageFlags = () => {
      st.windAmp = stageWind(st.stage) * st.w
      setStage(st.stage)
      setWindy(st.windAmp > 0)
    }
    ...
    const start = (cfg) => {
      const startStage = Math.max(1, Math.floor(Number(cfg && cfg.stage) || 1))
      const startScore = Math.max(0, Math.floor(Number(cfg && cfg.score) || 0))
      st.layers.length = 0
      st.debris.length = 0
      st.sparks.length = 0
      const bw = baseW()
      st.stage = startStage
      st.layers.push({ x: (st.w - bw) / 2, w: bw, ci: (st.stage - 1) % PALETTE.length, label: pickLabel(0) })
      st.mov = mkMov(1)
      st.score = startScore
      st.combo = 0
      st.stageLayers = 0
    ...
    const burst = (x, y, ci, n, gold) => {
      ...
      for (let i = 0; i < count; i++) {
        st.sparks.push({
          ...
          t: 0,
          col: gold ? (Math.random() < 0.5 ? '#ffd166' : '#ffe9a8') : (Math.random() < 0.5 ? c.cream : c.crumb),
        })
      }
    }
    ...
    const endGame = () => {
      ...
      reportProgress(true) // clears the resume point: a lost run is not resumed
      play('lose')
    }
    ...
    const clearStage = () => {
      ...
      st.hold = STAGE_HOLD
      st.mov = null
      st.celebrate = st.rm ? 0.4 : 1
      // small reward for depth: a sliver of width back and a stage bonus
      const top = st.layers[st.layers.length - 1]
      if (top) top.w = Math.min(baseW(), top.w + baseW() * 0.06)
      st.score += 120 + st.stage * 30
      syncStageFlags()
      setScore(st.score)
      setBanner({ stage: st.stage, name: stageName(st.stage) })
      burst(st.w / 2, st.baseY - st.layers.length * st.lh + st.cam, (st.stage - 1) % PALETTE.length, 26, true)
      play('win', { gain: 0.5 })
      reportProgress(false) // a resume point at the start of the new stage
    }

    const drop = () => {
      const m = st.mov
      const top = st.layers[st.layers.length - 1]
      if (!m || !top || st.hold > 0) return
      ...
      const dx = m.x - top.x
      const perfect = Math.abs(dx) <= Math.max(3, top.w * 0.024)
      let nx = left
      let nw = overlap

      if (perfect) {
        nx = top.x
        nw = Math.min(baseW(), top.w + 4)
        st.combo += 1
        st.flash = 1
        st.toast.t = 1
        st.toast.txt = 'مثالي'
        st.toast.x = nx + nw / 2
        st.toast.y = y
        st.score += 12 + 16 * Math.min(st.combo, 8)
        burst(nx + nw / 2, y, m.ci, 24, true)
      ...
      if (st.stageLayers >= LAYERS_PER_STAGE) {
        clearStage()
      } else {
    ...
    const drawLayer = (x, y, w, h, ci, label, alpha) => {
      ...
      rr(ctx, x, y, w, h, Math.min(7, h * 0.3))
      ctx.fill()
      ...
      rr(ctx, x, y, w, ch, Math.min(6, ch * 0.5))
      ctx.fill()
      const drips = Math.max(2, Math.min(6, Math.floor(w / 46)))
      ...
      if (w > 96 && h > 18 && label) {
        ...
        ctx.font = `700 ${Math.round(Math.min(15, h * 0.4))}px system-ui, "Segoe UI", Tahoma, sans-serif`
    ...
    const draw = () => {
      ...
      if (standY < h + 40) {
        ...
        rr(ctx, w * 0.5 - Math.min(w * 0.42, 220), standY, Math.min(w * 0.84, 440), 10, 5)
        ctx.fill()
        ctx.fillStyle = 'rgba(255,255,255,.07)'
        rr(ctx, w * 0.5 - 16, standY + 10, 32, Math.max(0, h - standY - 10), 4)
        ctx.fill()
      }
      ...
      // the sliding layer
      if (st.mov && st.phase === 'play' && st.hold <= 0) {
      ...
      if (st.toast.t > 0.01) {
        ...
        ctx.lineWidth = 3
        ctx.beginPath()
        ctx.arc(st.toast.x, st.toast.y - p * 26, 16 + p * 46, 0, Math.PI * 2)
        ctx.stroke()
        ctx.fillStyle = '#fff'
        ctx.font = '800 19px system-ui, "Segoe UI", Tahoma, sans-serif'
        ctx.textAlign = 'center'
        ctx.textBaseline = 'middle'
    ...
    const frame = (now) => {
      ...
      if (st.phase === 'play') {
        if (st.hold > 0) {
          ...
          const m = st.mov
          const wind = st.windAmp ? Math.sin(st.windT * 1.6) * st.windAmp : 0
          m.x += (m.dir * m.speed + wind) * dt
      ...
      st.celebrate = Math.max(0, st.celebrate - dt * 1.5)
    ...
    >
      ...
      {phase === 'play' && (
        <div className="gmx-hud">
          ...
          {windy && (
            <span className="gmx-pill arb-wind" aria-label="رياح">
              <svg viewBox="0 0 24 14" width="26" height="15" aria-hidden="true">
                <path d="M2 4h12a2.4 2.4 0 1 0-2.4-2.4M2 8h16a2.6 2.6 0 1 1-2.6 2.6M2 11h9" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" />
              </svg>
              رياح
            </span>
          )}
      ...
      {banner && phase === 'play' && (
        <div className="arb-banner" key={banner.stage}>
          ...
          <span className="arb-banner-n">{banner.name}</span>
        </div>
      )}
